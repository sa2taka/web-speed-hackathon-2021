...
const NewPostModalPage: React.VFC<Props> = ({ hasError, isLoading, onResetError, onSubmit }) => {
  ...
  const handleChangeImages = React.useCallback((ev) => {
    const files = Array.from<File>(ev.currentTarget.files).slice(0, 4);
  ...
  return (
    <section>
      <form className="flex flex-col items-center w-full" onSubmit={handleSubmit}>
        <textarea
          className="p-4 w-full h-24 placeholder-gray-300 rounded border border-gray-300 resize-none"
          onChange={handleChangeText}
          placeholder="いまなにしてる？"
        />
        <p className="flex justify-evenly items-center mt-4 w-full text-gray-900">
